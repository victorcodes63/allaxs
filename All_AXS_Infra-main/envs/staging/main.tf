...
############################
# Managed PostgreSQL (placeholder)
############################
...
# NOTE: For dev-only convenience; in prod prefer passwords via secrets.
resource "digitalocean_database_user" "pgpass_user" {
  cluster_id = digitalocean_database_cluster.pg.id
  name       = "${var.db_user}_pw"
...
# CDN for assets: cdn-staging.example.com → DO CDN endpoint
...
############################
# App Platform (stub) — web, api, worker
############################
# NOTE: This is a stub. It uses public images for now; switch to DOCR images after CI pushes.
#
...
#   - TODO: confirm Cloudflare SSL mode is 'Full (strict)' in production account
