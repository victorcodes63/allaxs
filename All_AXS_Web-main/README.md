...
**Next.js 16 proxy:** Route protection lives in `proxy.ts` at the project root (same level as `app/`). Next.js 16 renamed the old `middleware.ts` convention; keep the file here, not under `app/`.
...
The onboarding page is protected by the root proxy (`proxy.ts`):
...
The following path prefixes are protected by `proxy.ts`:
...
5. **Protected routes** check for `accessToken` cookie in `proxy.ts`; if missing or expired, redirect to login
...
3. **Environment Variables**
   ...
   **Note:** 
   ...
   - `API_URL` is used by Next.js API routes (server-side proxies) and Cypress `cy.env('API_URL')` for seed helpers
   - `WEB_BASE_URL` sets Cypress `baseUrl` (see `cypress.config.ts`)
   - Set `NEXT_PUBLIC_API_MOCK=1` to enable Next.js proxy routes for ticket types (disabled by default)
   
   **Stubbed auth email specs** (`auth-email-flows.cy.ts`, `forgot-password.cy.ts`, `verify-email.cy.ts`, `reset-password.cy.ts`) intercept `/api/auth/*` in the browser — only the Next dev server is required; no live Resend or backend test routes.

   **Live API specs** (`auth-e2e.cy.ts`, `rbac-e2e.cy.ts`, etc.) need the backend running with `ENABLE_TEST_ROUTES=true` or `NODE_ENV=test`.
...
- `cypress/e2e/auth-email-flows.cy.ts` - Forgot password, register/check-email, verify/resend (stubbed API)
