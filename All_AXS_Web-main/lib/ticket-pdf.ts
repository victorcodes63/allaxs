...
/** Keep in sync with API `src/tickets/ticket-pdf.layout.ts` (dimensions & colors). */
const C = {
...
function fitToMaxBox(naturalW: number, naturalH: number, maxW: number, maxH: number) {
...
function measureHeaderHeight(
  ...
  const tierBlock = 30;
  return Math.max(124, 18 + HEADER_LOGO_SLOT_H + 18 + titleBlock + tierBlock + 14);
}

function drawHeaderBrandText(doc: JsPdf, x: number, y: number) {
  doc.setFont("helvetica", "bold");
  doc.setFontSize(9);
...
function drawWordmarkText(doc: JsPdf, cx: number, y: number) {
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8);
  doc.setTextColor(...C.muted);
...
function drawHeader(
  ...
  if (assets.logoWhiteUrl && assets.logoWhiteFit) {
    try {
      ...
      doc.addImage(assets.logoWhiteUrl, "PNG", x + pad, logoDrawY, drawW, drawH);
    } catch {
...
function renderTicketPdf(doc: JsPdf, content: TicketPdfContent, assets: TicketPdfAssets) {
  ...
  doc.addImage(assets.qrDataUrl, "PNG", qrBlockX + 12, qrBlockY + 12, qrSize, qrSize);
...
/** Branded ticket PDF — layout matches email attachment (API ticket-pdf.layout.ts). */
export async function downloadTicketPdf(
  content: TicketPdfContent,
  options: { origin: string; fileBaseName?: string; ticket: StoredTicket }
): Promise<void> {
  const { jsPDF } = await import("jspdf");

  const qrPayload = buildTicketQrUrl(options.origin, options.ticket);
  ...
  let logoWhiteFit: { w: number; h: number } | null = null;
  if (logoWhiteUrl) {
    try {
      const { w, h } = await measureImage(logoWhiteUrl);
      logoWhiteFit = fitToMaxBox(w, h, 88, 22);
    } catch {
      logoWhiteFit = null;
    }
  }
  ...
  renderTicketPdf(doc, content, { qrDataUrl, logoWhiteUrl, logoWhiteFit });
