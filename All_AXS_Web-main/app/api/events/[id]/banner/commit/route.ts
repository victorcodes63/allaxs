...
export async function POST(
  ...
  try {
    ...
    const endpoint = `${API_URL}/events/${eventId}/banner/commit`;
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        return NextResponse.json(
          {
            message: data.message || "You do not have permission to commit banner for this event",
          },
          { status: 403 }
        );
      }
      ...
      return NextResponse.json(
        {
          message:
            data.message || `Failed to commit banner (${response.status})`,
        },
    ...
    console.error("Commit banner error:", error);
    return NextResponse.json(
      {
        message:
          (error as Error).message ||
          "An error occurred while committing the banner",
      },
      { status: 500 }
    );
  }
