  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        return NextResponse.json(
          {
            message: data.message || "You do not have permission to access this event",
          },
          { status: 403 }
        );
      }
      ...
      return NextResponse.json(
        {
          message: data.message || `Failed to fetch event (${response.status})`,
        },
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        return NextResponse.json(
          {
            message: data.message || "You do not have permission to edit this event",
          },
          { status: 403 }
        );
      }
      ...
      return NextResponse.json(
        {
          message: data.message || `Failed to update event (${response.status})`,
        },
        { status: response.status }
      );
    }
