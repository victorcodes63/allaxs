...
Production on Vercel uses **`api/index.ts`** (exact `/api`) plus **`api/[...segments].ts`** (all `/api/*` subpaths) re-exporting the same Nest bootstrap from **`src/vercel/nest-serverless.entry.ts`**, together with **`vercel.json` `rewrites`**: **`/:path*` → `/api/:path*`** so `/auth/login` is invoked as `/api/auth/login` on the catch-all function (Vercel does **not** expand **`$1`** in JSON destinations — a literal `/api/$1` breaks routing). The handler strips the `/api` prefix (and catch-all query noise) so Nest sees `/auth/login`. **Do not** use **`api/[[...segments]].ts` (optional catch-all)** on current Vercel: nested paths like `/api/auth/login` may never hit the function (platform `NOT_FOUND`). **Do not** rewrite to `/api/:segment/:path*` — that makes Vercel look for non-existent files like `api/auth/login.ts` (`NOT_FOUND`). The Vercel project’s **Root Directory** should be **`.`** (the backend repo root).
