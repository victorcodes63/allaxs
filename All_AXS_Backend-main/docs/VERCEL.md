...
Production on Vercel uses **`api/[[...segments]].ts`** plus **`vercel.json` `rewrites`**: **`/:path*` → `/api/:path*`** so `/auth/login` is invoked as `/api/auth/login` on the catch-all function (Vercel does **not** expand **`$1`** in JSON destinations — a literal `/api/$1` breaks routing). The handler strips the `/api` prefix (and optional `[...segments]` query noise) so Nest sees `/auth/login`. **Do not** rewrite to `/api/:segment/:path*` — that makes Vercel look for non-existent files like `api/auth/login.ts` (`NOT_FOUND`). The Vercel project’s **Root Directory** should be **`.`** (the backend repo root).
